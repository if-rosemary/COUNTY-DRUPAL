...
2. Now, checkout your new story branch. The branch name convention is ***activity/WCOR-1001*** (the story ID number without leading zeroes comes after "WCOR-"). Possible activities include:
	...
	*(story ID in the format: WCOR-1001)*
...
2. Stage those changes for the next commit to your local story branch on Cloud IDE, and then commit them. (This can be done multiple times a day, or just once at the end.)
	...
	d. Commit the changes you've staged to your local Cloud IDE story branch. Use "WCOR-" and the story ID number **without leading zeroes** at the start of your commit message (e.g. WCOR-1001). Write your commit message in present tense starting with an action verb to describe the latest round of changes. Example:

	``` git commit -m "WCOR-[story-ID-number]: Add content type Page." ```

	Note: Unplanned (non-story) work like hotfixes should be entered under ID ``` WCOR-0 ```.
...
2. Use the BLT Check function as a pre-deployment check to ensure there are no configuration discrepancies local YML files and database, no pending Drupal security updates, and no critical Composer dependency updates:

``` blt custom:check ```

If an exit code or error appears, an adjustment will be needed.

For config mismatch, an adjustment to the feature related to the named YML file may be needed, with a ``` drush cex ``` to follow. For Drupal security updates or Composer dependency updates, it will require a ``` composer update ``` of the named Drupal or other related dependency (e.g. drupal/webform, phpmailer/phpmailer, etc.)

Check with other team members if you need assistance to resolve.

3. Take one last opportunity to execute the existing test plan (from the ServiceIT story acceptance criteria field) in your local dev site. You are likely to find that the plan could use more specific detail to ensure a quality result. Update the test plan accordingly.

4. When the feature or story item is considered complete, create and submit a pull request (PR).
...
5. Acquia Pipelines will run against the newly submitted PR. This includes a number of automated technical checks along with any Behat tests that have been written.
...
6. Once someone has signed off on QA, the Build Coordinator will attempt to **merge** the PR. Our workflow designates the Build Coordinator to merge the pull request. Squash-and-merge will be the standard merge method.
