...
class FeatureContext extends RawDrupalContext {
   ...
   *
   * Examples:
   * Given the "slide" paragraph type exists
   *
   * @throws \Exception;
   */
  ...
  public function roleOnlyPermissions(TableNode $rolesTable) {
    ...
    $defined_permissions = [];
    foreach ($rolesTable as $rolePermission) {
      $this->roleHasPermission($rolePermission['role'], $rolePermission['permission']);
      $allowed_roles[] = $rolePermission['role'];
      $defined_permissions[] = $rolePermission['permission'];
    }
    $allowed_roles[] = 'administrator';
    $defined_permissions = array_unique($defined_permissions);
    $all_roles = $this->getRoles();
  ...
  /**
   * @Then I should be able to edit a/an :type(vocabulary) term
   */
  public function assertEditTermOfType($type) {
    $term = (object) array(
    ...
    $this->getSession()->visit($this->locatePath('/taxonomy/term/' . $saved->tid . '/edit'));
  ...
  /**
   * Asserts that a given content type with a given section is editable.
   *
   * @Then I should be able to edit a/an :type( content) with the editorial section :section
   */
  public function assertEditNodeOfTypeInSection($type, $section) {
    $node = (object) array(
      'type' => $type,
      'title' => "Test $type",
      'field_editorial_sections' => $section
    );
  ...
  /**
   * @Then /^I should be able to get tweets for \'([^\']*)\'$/
   */
  public function iShouldBeAbleToGetTweetsFor($handle) {
    $tweets = mwaa_social_get_tweets($handle, 1);

    if (empty($tweets)) {
      throw new ExpectationFailedException('Unable to get tweets for ' . $handle);
    }
  }
